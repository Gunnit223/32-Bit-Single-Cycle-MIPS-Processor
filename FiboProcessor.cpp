...
void lwFSM(void) {
    ...
    int32_t data = readDMM(ALU(out_reg1, signExtn(Immediate)), true);
    writeRegfile(RT, true, &data);
}
...
void addiFSM(void) {
	...
	int32_t out_reg1, out_reg2, out_reg;
	//Write your code here, without declaring extra variables
    readRegfile(RS, 0, &out_reg1, &out_reg);
...
void bneFSM(void) {
    ...
    if (!ZeroFlag) {
        PC = addBranchAddr(PC, leftShift2_16(Immediate));
    }
